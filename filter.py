...
##    st.session_state.text = 'Text bytes:   '+arr+ '\n'

    # Channel A and B plots
    ax1.plot(list(range(len(x))), x, label='input', alpha=0.4)
    ax1.plot(list(range(len(x))), y_no_round, label='not rounded', color='orange', linewidth=5)
    ax1.plot(list(range(len(x))), y_hw, label='Hardware', color='green', marker="x")
...
##    ax1.axhline(-1.*int(1. * float(threshold)), color='orange', linestyle='--', label='BOFF')
    ax1.set_ylabel('LSBss #')
    ax1.set_xlabel('sample number, includes oversampling')
    ax1.set_title('filtered sig.: Ampl.: '+str(amp)+' , oversampl.: ' + str(ov)+' BW: '+str(round(f_3db, 4)))
